...
int main(void)
{
	...
	#ifdef REV2_1
		alpha_displayOverlayMessage("    ConFLiCT                    FW: C0.5 HW: 2.1");
	#endif
	#ifdef REV2_2
		alpha_displayOverlayMessage("    ConFLiCT                    FW: C0.5 HW: 2.2"); 
	#endif	
	...
	while(1)						// main loop
    {
		...
		// now look at the alerts
		if (alert_status.overtemp)		// warning overtemp
		{
			// write display warning here maybe
		}
		if (alert_status.fanblock)		// warning blocked fan
		{
		}
		if (alert_status.lowWaterFlow)	// low/no water flow
		{
		}
				...
				switch(aDataSet.id)
				{
					case 01:							// reset
						if (aDataSet.index == 170 && aDataSet.data == 85)
						{
							...
							CPU_CCP = 0xD8;				// disable write protection
							RST_CTRL = 1;				// do a software reset
